...
[`.claude/CLAUDE.md`](../../bundle/.claude/CLAUDE.md) — 149 строк, ~3k
...
- Имена ссылок-вложений восстанавливаются из атрибута
  ...
  не `download.xhtml?...`.
- Цвет со смыслом доезжает как `<span style="color:#hex">`, цвета темы
  нет. Ссылка, которую Confluence не отрисовал, сохраняет слова, вшитая
  `data:`-картинка становится файлом, ручной markdown склеивается в одну
  ссылку, относительная ссылка вне выгрузки получает хост из `--base-url`.
...
- `stage-brief.sh` (`SessionStart` на `resume|compact`) — одна строка с путём
  к брифу, если есть стадия с `brief.md` и без `report.md`; иначе молчит.
  Бриф и критерии готовности перестают теряться после compact в длинной
  сессии.
